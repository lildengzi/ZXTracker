...
fn handle(
    ...
    let resp = match req.cmd.as_str() {
        ...
        _ => IpcResponse {
            ok: false,
            error: format!("unknown cmd: {}", req.cmd),
            data: None,
        },
    };
...
fn handle_status(state: &Arc<Mutex<SharedState>>, db: &Database) -> IpcResponse {
    ...
    let usage: Vec<serde_json::Value> = s
        .today_usage
        .iter()
        .map(|(a, t)| {
...
fn handle_get_today(db: &Database) -> IpcResponse {
    ...
    match db.today_usage(&today) {
        Ok(usage) => {
            let data: Vec<serde_json::Value> = usage
                .iter()
                .map(|(a, t, c)| serde_json::json!({"app_id": a, "seconds": t, "count": c}))
                .collect();
            IpcResponse {
                ok: true,
                error: String::new(),
                data: Some(serde_json::json!({"date": today, "usage": data})),
            }
        }
        Err(e) => IpcResponse {
            ok: false,
    ...
    match db.range_usage(&from, &today) {
        Ok(usage) => {
            ...
            IpcResponse {
                ok: true,
                error: String::new(),
                data: Some(serde_json::json!({"from": from, "to": today, "usage": data})),
            }
        }
        Err(e) => IpcResponse {
            ok: false,
...
fn handle_get_hourly(db: &Database) -> IpcResponse {
    ...
    match db.range_hourly_usage(&today, &today) {
        Ok(data) => {
            let mut per_app: std::collections::HashMap<String, [i64; 12]> =
            ...
            let result: Vec<serde_json::Value> = per_app
                .iter()
                .map(
                    |(app, buckets)| serde_json::json!({"app_id": app, "hourly": buckets.to_vec()}),
                )
                .collect();
            IpcResponse {
                ok: true,
                error: String::new(),
                data: Some(serde_json::json!({"hourly": result})),
            }
        }
        Err(e) => IpcResponse {
            ok: false,
    ...
    match db.list_tags() {
        Ok(tags) => {
            let data: Vec<serde_json::Value> = tags
                .iter()
                .map(|(name, apps)| serde_json::json!({"name": name, "apps": apps}))
                .collect();
            IpcResponse {
                ok: true,
                error: String::new(),
                data: Some(serde_json::json!({"tags": data})),
            }
        }
        Err(e) => IpcResponse {
            ok: false,
